...
const Timeline = {
  ...
  renderPings() {
    ...
    el.innerHTML = `
      <p class="text-xs text-gray-500 mb-4">点击「打卡」按钮从底部抽屉选择联系人，支持多选与搜索；过去 7 天可补记。</p>
      ...
      <div class="mb-6">
        <div class="flex items-center justify-between mb-3">
          ...
          <button onclick="Timeline.openPingSheet('${todayStr}')" class="btn-primary text-xs px-4 py-2 flex items-center gap-1.5">
            <span class="text-base leading-none">＋</span> 打卡
          </button>
        </div>
        <div class="min-h-[60px] p-3 rounded-xl border border-white/5 bg-white/[0.02] flex flex-wrap gap-2 items-start">
          ...
          `).join('')}
          ${(this.pings[todayStr] || []).filter(matchFilter).length === 0 ? '<span class="text-xs text-gray-600 py-2">今日暂未打卡，点击右上「打卡」选择联系人</span>' : ''}
        </div>
      </div>

      <!-- Recent history -->
      <div>
        <h3 class="text-sm font-semibold text-gray-400 mb-3">近期记录 <span class="text-[10px] text-gray-600 font-normal">（点击「补记」回填过去日）</span></h3>
        <div class="space-y-3">
            ...
            return `
              <div class="flex items-start gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5">
                ...
                <div class="shrink-0 flex items-center gap-2">
                  ...
                  <button onclick="Timeline.openPingSheet('${dateStr}')" class="text-[11px] px-3 py-1.5 rounded-md bg-neon-blue/10 border border-neon-blue/20 text-neon-blue hover:bg-neon-blue/20 transition-all" title="补记该日浅社交">＋ 补记</button>
                </div>
              </div>
            `;
          }).join('')}
  ...
  // ── Bottom-sheet contact picker (mobile-friendly add) ──
  _pingSheetState: null,

  openPingSheet(dateStr) {
    const pingedIds = new Set((this.pings[dateStr] || []).map(p => p.contact_id));
    const isToday = dateStr === Utils.localDateStr(new Date());
    this._pingSheetState = { dateStr, selected: new Set(), search: '', pingedIds };

    // Remove existing sheet if any
    document.getElementById('ping-sheet-overlay')?.remove();

    const overlay = document.createElement('div');
    overlay.id = 'ping-sheet-overlay';
    overlay.className = 'bottom-sheet-overlay';
    overlay.innerHTML = `
      <div class="bottom-sheet" id="ping-sheet">
        <div class="bottom-sheet-handle"></div>
        <div class="bottom-sheet-head">
          <div class="flex items-center justify-between mb-2">
            <div>
              <div class="text-sm font-semibold text-white">${isToday ? '今日打卡' : '补记 · ' + dateStr}</div>
              <div class="text-[11px] text-gray-500 mt-0.5">点选联系人后底部确认；已打卡的会自动置灰</div>
            </div>
            <button onclick="Timeline.closePingSheet()" class="text-gray-500 hover:text-white text-xl px-2">✕</button>
          </div>
          <input id="ping-sheet-search" class="form-input w-full text-sm" placeholder="🔍 搜索联系人姓名 / 公司"
            oninput="Timeline._onPingSheetSearch(this.value)" autocomplete="off">
        </div>
        <div class="bottom-sheet-body" id="ping-sheet-list">
          ${this._renderPingSheetList()}
        </div>
        <div class="bottom-sheet-foot">
          <button type="button" onclick="Timeline.closePingSheet()" class="btn-ghost flex-1">取消</button>
          <button type="button" onclick="Timeline.confirmPingSheet()" class="btn-primary flex-[2]" id="ping-sheet-confirm">
            打卡 <span id="ping-sheet-count">0</span> 人
          </button>
        </div>
      </div>
    `;
    ...
    overlay.addEventListener('click', (e) => {
      if (e.target === overlay) this.closePingSheet();
    });
    // Focus search on desktop only (mobile can be annoying)
    if (!('ontouchstart' in window)) {
      setTimeout(() => document.getElementById('ping-sheet-search')?.focus(), 50);
    }
  },

  _renderPingSheetList() {
    const { selected, search, pingedIds } = this._pingSheetState;
    ...
    if (filtered.length === 0) {
      return '<div class="text-xs text-gray-500 text-center py-8">未找到匹配的联系人</div>';
    }
    return `<div class="flex flex-wrap gap-2">
      ${filtered.map(c => {
        const already = pingedIds.has(c.id);
        const sel = selected.has(c.id);
        const cls = already ? 'bottom-sheet-pill disabled' : (sel ? 'bottom-sheet-pill selected' : 'bottom-sheet-pill');
  ...
  _onPingSheetSearch(val) {
    if (!this._pingSheetState) return;
    this._pingSheetState.search = val;
    const list = document.getElementById('ping-sheet-list');
    if (list) list.innerHTML = this._renderPingSheetList();
  },

  _togglePingSheetPick(cid) {
    if (!this._pingSheetState) return;
    const { selected } = this._pingSheetState;
    if (selected.has(cid)) selected.delete(cid);
    else selected.add(cid);
    // Patch the affected pill in-place to avoid re-render scroll jump
    const pill = document.querySelector(`#ping-sheet-list [data-cid="${cid}"]`);
    if (pill && !pill.classList.contains('disabled')) {
      pill.classList.toggle('selected', selected.has(cid));
    }
    const countEl = document.getElementById('ping-sheet-count');
    if (countEl) countEl.textContent = selected.size;
  },

  closePingSheet() {
    document.getElementById('ping-sheet-overlay')?.remove();
    this._pingSheetState = null;
  },

  async confirmPingSheet() {
    if (!this._pingSheetState) return;
    const { dateStr, selected } = this._pingSheetState;
    const ids = Array.from(selected);
    if (ids.length === 0) {
      Utils.toast('请至少选择 1 位联系人', 'error');
      return;
    }
    ...
    this.renderPings();
    if (fail === 0) {
      Utils.toast(`已打卡 ${ok} 人`);
    } else if (ok === 0) {
      Utils.toast(lastErr || `打卡失败`, 'error');
    } else {
      Utils.toast(`成功 ${ok} 人，${fail} 人失败：${lastErr}`, 'error');
    }
  },
